...
fomra en que se tiene acceso a los campos de los apuntadores de tipo estructura
...
{
int matricula;
char nombre[20];                             /* Campos de la estructura alumno. */
char carrera[20];
float promedio;
char direccion[20];
...
void main(void)
{
struct alumno a0 = {120, "Maria", "Contablidad, 8.9, "Queretaro"};
struct alumno *a3, *a4, *a5, a6;
/*Observa que las variables *a3, *a5 y *a5 se declara de tipo estructura alumno.*/

a3 =&a0;            /* En este caso el apuntador de tipo estructura alumno a3
se le asigna la direccion de la variable de tipo estructura alumno, a0. */

a4 = new (struct alumno);
/*Nota el apuntador a4 es necesario asignarle una direccion de memoria.
...
formatos. */
printf("\nIngrese la matricula de alumno 4: ");
scanf("%d", &(*a4). maricula);
fflush(stdin);
printf("\nIngrese el nombre de alumno 4: ");
gets(a4->nombre);
...
Lectura(a5);     /*En este caso se pasa el apuntador de tipo estructura alumni
...
direccion para predecer a la variable. */
printf("\nDatos del alumno 3\n");
/*Observa la foram de escribir los campos de los apuntadores de tipo
estructura. */

printf("%d\t%s\t%s\t%.2f\t%s", a3-> matricula, a3->matricula, a3->nombre, a3->carrera,
...
printf("%d\t%s\t%s\t%.2f\t%s", a4->matricula, a4->nombre, a4->carrera,
       ...
       printf("Datos de alumno 5\n")
       printf("%d\t%s\t%s\t%.2f\t%s", a5->matricula, a5->nombre, a5->carrera,
                a5->promedio, a5->direccion);

printf("\nDatos del alumno 6\n");
/*Observa la forma de escribir los campos de la variable de tipo estructura. */
