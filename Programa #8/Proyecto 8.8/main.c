...
mujeres registradores en el hospital. */
{
int I, FEM, MAS, TOT;
for (I=0; I<T; T++)
   switch (A[I].sexo)
...
TOT = FEM + MAS;
printf("\nPorcentaje de Nombres: %.2f%", (float)MAS / TOT * 100);
printf("\nPorcentaje de Mujeres: %.2f%", (float)FEM / TOT * 100);
}

void F2(paciente A[], int T)
/* Esta funcion se utiliza para obtener el numero de pacientes que ingresarion al
...
printf("\nNumeros pacientes en condicion 1: %d", C1);
printf("\nNumeros pacientes en condicion 2: %d", C2);
printf("\nNumeros pacientes en condicion 3: %d", C3);
printf("\nNumeros pacientes en condicion 4: %d", C4);
printf("\nNumeros pacientes en condicion 5: %d", C5);
}
...
for (I=0; I<T; I++)
  if (A[I].con ==5)
         printf("\nNombres: %s\tTelefono: %s", A[I].nom, A[I].tel);
}
