    ...
    switch (OPE)
    {
        ...
        case 2: Rebastecimiento(INV, TAM);
         ...
         case 3: Nuevos_Productos(INV, TAM);
           /* Se pasa el parametro por referencia, por que se puede modificar e;
           numero
...
for (I=0; I<T; I++)
{
    printf("nIngrese informacion del producto %d", I+1);
    printf("\n\tClave: ");
    scanf("%d", &A[I].clave);
    fflush(stdin);
    printf("\tNombre:");
...
/* Esta function se utiliza para manejar las ventas a un cliente. Se ingresan
producto y cantidades, el fin de datos esta dado por el cero. Ademas de
obtener el total de la Ventas, se actualiza el Inventario. */
{
int CLA, CAN, I, RES;
float TOT, PAR;
...
{
        A[I].existencia -= CAN;     /* Se actualiza el stock el producto. */
        PAR = A[I].precio * CAN;
        TOT += PAR;
...
{
 printf("\nNo existen en inventario la cantidad solicitada. Solo hay %d"
            A[I].existencia);
  printf(" \nLos llevas 1 - Si 0 -NO?: ");
  scanf("%d", &RES);
  if (RES)
  {
 ...
 printf("\nIngresa la siguiente clave del producto -0 para salir -:);
 scanf("%d", &CLA);
...
/*Esta funcion se utiliza para incorporar nuevos producto al Inventario.
Dado que los producto que se encuentra ordenadores por clave, puede suceder que
al intertar un nuevos productos haya que mover los elementos del arrelo para
...
if (I == *T)                /* Se inserta el elemento en la ultima posicion. */
{
    ...
    *T = *T + 1;
    ...
    if(A[I].clave == CLA)
     printf("\nEl producto ya encuentra en el inventario");
    else
    {
      for (J=*T; J>I; J--)
       /* Se inserta el nuevo producto en el arreglo. Se mueve una posicion
     a la derecha los elementos
            del arreglo que tenga una clave del producto mayor a la ingresada. */
                A[J] = A[J-1];
            A[I].clave = CLA;
            printf("\tNombre:");
            fflush(stdin);
