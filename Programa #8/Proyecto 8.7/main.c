...
matpro cal[5];      /* Obserca que cal es un arreglo unidimensional de tipo
                        estructura
} alumno;                             matpro - la estructura definida en primer termino. */
...
/* Esta funcion se utiliza para leer la informacion de una arreglo unidimensional
...
void F2(alumnoA[], int T)
/* Esta funcion se utiliza para obtener la matriculas de los alumnos cuya
calificacion en la tercera materia es amyor a 9. */
{
int I;
...
float SUM = 0.0;
for (I=0; I<T; I++)
 SUM = SUM + A[I].cla[3].pro;
SUM = SUM / T;
printf("\n\nPromedui de la cuarta materia: %.2f", SUM);
}
