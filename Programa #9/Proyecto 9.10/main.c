...
El segundo señala el numero de bloques que debe moverse, en terminos
...
que se debe mover a partir que la posicion en la que actualmente se
encuentra y 2 para indicar que el movimiento es partir del fin del
archivo. */
...
scanf("%f", &alu.promedio);     /* Modifica el registro com el
                                    nuevo promedio. */
 fseek(ap, (d-1)*sizeof(alumno), 0);
 /*Nos tenemis que posicionar nuevamente en el lugar correcto para
 escribir el registro modificado. Observa que si no hacemos este
 respocionamiento escribiriamos el registro actualizado en la siguiente posicion. */
